...
if __name__ == "__main__":
    ...
    # 1. Hide your token securely using Streamlit Secrets
    api_token = st.secrets["FLIGHTLOGGER_TOKEN"]

    # 2. Fetch the comments (with a button instead of automatically)
    if st.button("Fetch Today's Flights"):
        comments = get_todays_training_comments(api_token)
        st.session_state['comments'] = comments # Save it in the browser's memory

    # 3. Create the UI if comments were found
    if 'comments' in st.session_state and st.session_state['comments']:
        comments = st.session_state['comments']
        student_names = list(comments.keys())
        
        # Replaces your terminal input() with a nice dropdown multi-select
        selected_students = st.multiselect("Select students to include:", student_names)
        
        # Replaces your height input() with a numeric slider/box
        user_height = st.number_input("Max rectangle height (cm):", min_value=1.0, value=8.0, step=0.5)
        
        if st.button("Generate PDF"):
            # We save the PDF to a virtual memory buffer instead of the hard drive
            pdf_buffer = io.BytesIO() 
            
            # You would slightly modify your create_constrained_pdf function to accept pdf_buffer 
            # instead of an output_pdf_path string, and only process 'selected_students'
            create_constrained_pdf(comments, pdf_buffer, user_height, selected_students)
            
            # Provide a download button to the user
            st.download_button(
                label="Download PDF",
                data=pdf_buffer.getvalue(),
                file_name="debriefings.pdf",
